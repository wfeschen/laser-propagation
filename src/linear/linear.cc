...
namespace Linear {
  ...
  double Base::group_velocity(double kperp, double omega, double p) const {
    double domega = 1e-2 * omega;
    // five-point stencil to compute the derivative d/dw k(w)| w = w0
    double kp = (-kz(kperp, omega+2*domega, p) + 8.0*kz(kperp, omega+domega, p) - 8.0*kz(kperp, omega-domega, p) + kz(kperp, omega-2*domega, p)).real() / (12*domega);
    return 1/kp;
  ...
  std::complex<double> FreeSpace::kz(double kperp, double omega, double p) const {
    std::complex<double> index = Medium::pressurize(p, n, omega);
  ...
  std::complex<double> DiffractionLess::kz(double, double omega, double) const {
    std::complex<double> index = n(omega);
    double k0 = omega / Constants::c;
    double k = std::real(index) * k0;
    double alpha = 2*std::imag(index) * k0;
  ...
  std::complex<double> Capillary::kz(double kperp, double omega, double) const {
    double k0 = omega / Constants::c;
