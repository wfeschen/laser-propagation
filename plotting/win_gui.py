...
import subprocess
import numpy as np
import intensity, log_spectrum, max_density, max_intensity
import spectrum, thz_spectrum, temporal, onaxis, energy, spectral, density
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.temporal['command'] = lambda: temporal.plot(self.input_file.get(), float(self.distances.get()))
        self.temporal.grid(row=3, column=0)
        
        self.onaxis = ttk.Button(self.bottom, text='E(r=0,t)')
        self.onaxis['command'] = lambda: onaxis.plot(self.input_file.get(), float(self.distances.get()))
        self.onaxis.grid(row=4, column=0)

        self.intensity = ttk.Button(self.bottom, text='I(r,t)')
        self.intensity['command'] = lambda: intensity.plot(self.input_file.get(), float(self.distances.get()))
        self.intensity.grid(row=5, column=0)
        
        self.max_intensity = ttk.Button(self.bottom, text='Max I(z)')
        self.max_intensity['command'] = lambda: max_intensity.plot(self.input_file.get())
        self.max_intensity.grid(row=6, column=0)

        self.energy = ttk.Button(self.bottom, text='Energy(z)')
        self.energy['command'] = lambda: energy.plot(self.input_file.get())
        self.energy.grid(row=7, column=0)


        # second column
        self.spectral = ttk.Button(self.bottom, text='A(k,\u03c9)')
        self.spectral['command'] = lambda: spectral.plot(self.input_file.get(), float(self.distances.get()))
        self.spectral.grid(row=3, column=1)
        
        self.spectrum = ttk.Button(self.bottom, text='S(\u03bb)')
        self.spectrum['command'] = lambda: spectrum.plot(self.input_file.get(), float(self.distances.get()))
        self.spectrum.grid(row=4, column=1)

        self.log_spectrum = ttk.Button(self.bottom, text='log S(\u03bb)')
        self.log_spectrum['command'] = lambda: log_spectrum.plot(self.input_file.get(), float(self.distances.get()))
        self.log_spectrum.grid(row=5, column=1)

        self.thz_spectrum = ttk.Button(self.bottom, text='log S(THz)')
        self.thz_spectrum['command'] = lambda: thz_spectrum.plot(self.input_file.get(), float(self.distances.get()))
        self.thz_spectrum.grid(row=6, column=1)


        # third column
        self.density = ttk.Button(self.bottom, text='\u03c1(r,t)')
        self.density['command'] = lambda: density.plot(self.input_file.get(), float(self.distances.get()))
        self.density.grid(row=3, column=2)

        self.max_density = ttk.Button(self.bottom, text='Max \u03c1(z)')
        self.max_density['command'] = lambda: max_density.plot(self.input_file.get())
